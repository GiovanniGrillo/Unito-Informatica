    ...
    for ( i = 0; i < SO_MERCI; ++i) {
        for ( j = 0; j < var->n_porti; ++j) {
            if (porti[j].scadenza[giorno][i] != 0) {
                int diff = porti[j].offerta[i] - porti[j].scadenza[giorno][i];
                if (diff < 0) {
                    printf(":: --- NEGATIVA - (%d - %d) merce[%d](t_vita = %d) in porto[%d] (offerta)\n", porti[j].offerta[i], porti[j].scadenza[giorno][i], i + 1, merci[i].t_vita, j + 1);
                }
        ...
        for (j = 0; j < var->n_navi; ++j) {
            if (navi[j].scadenza[giorno][i] != 0 && navi[nave].pidNavi != -1) {
                int diff = navi[j].stiva[i] - navi[j].scadenza[giorno][i];
                if (diff < 0) {
                    printf(":: --- NEGATIVA - (%d - %d) merce[%d](t_vita = %d) in nave[%d]\n", navi[j].stiva[i], navi[j].scadenza[giorno][i], i + 1, merci[i].t_vita, j + 1);
...
int creazione_dom() {
    ...
    for (i = 0; i < var->SO_fill / SO_DAYS ;) {
        ...
        /* RANDOM QUANTITA' */
        q_ton = TonSize(merci[t_merce].size, var->SO_fill / SO_DAYS - i);
        if( q_ton  > (var->SO_fill / SO_DAYS - i) ) {
            return -1;
            }
...
int mareggiata() { 
    if(var->setOnMaraggiata == FALSE) return 0;
    ...
    if(semctl(semMareggiata, p_rand, GETVAL) == 0) 
            return 1;
