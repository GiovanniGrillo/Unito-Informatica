...
void createIPCS(char* file) {
    ...
    if ((shm_vars = shmget(ftok(FTOK_FILE, 'a'), sizeof(Var)*1000000*sizeof(int), IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1) ERROR;
    ...
    if ((sem_var         = semget(ftok(FTOK_FILE, 'v'), 1,                                            IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1) ERROR;
    if ((sem_atom        = semget(ftok(FTOK_FILE, 'k'), 1,                                            IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1) ERROR;
    if ((sem_inhibitor   = semget(ftok(FTOK_FILE, 'q'), 1,                                            IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1) ERROR;
...
int create_atom_init(int n_atoms){
    int i;
    pid_t pid;
    ...
    printf("%d atoms initials created\n",i);
}
void daily_log() {
    printf("\nEnd\n");
